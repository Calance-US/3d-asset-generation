...
class PromptSelector:
    ...
    def initialize_index(self) -> None:
        ...
        try:
            ...
            if not self.prompts:
                logger.info("No prompts found in database", extra={"action": "initialize_index"})
                return

            # Filter out prompts without embeddings
            prompts_with_embeddings = [p for p in self.prompts if p.embedding is not None]
            if not prompts_with_embeddings:
                logger.info("No prompts with embeddings found", extra={"action": "initialize_index"})
                return
            ...
            embeddings = np.array([p.embedding for p in prompts_with_embeddings], dtype=np.float32)
    ...
    def select_prompt(self, user_query: str, subject: str | None = None) -> tuple[str, str]:
        """Select the most appropriate prompt based on the user query and subject."""
        try:
            if not self.index or not self.prompts:
                logger.warning("Index not initialized or no prompts available", extra={
                    "action": "select_prompt",
                    "reason": "no_index_or_prompts"
                })
            ...
            if subject:
                ...
                filtered_prompts = [p for p in self.prompts if p.subject.lower() == subject.lower()]
                if not filtered_prompts:
                    ...
                    return subject.lower(), 'generic'  # Return the requested subject with generic type

                # Create mapping of filtered indices to original indices
                filtered_indices = [i for i, p in enumerate(self.prompts) if p.subject.lower() == subject.lower()]
                ...
                filtered_index.search(n=1, x=query_embedding.numpy(), k=k, distances=distances, labels=labels)
                ...
                distances = np.zeros((1, k), dtype=np.float32)
                labels = np.zeros((1, k), dtype=np.int64)
                self.index.search(n=1, x=query_embedding.numpy(), k=k, distances=distances, labels=labels)
                indices = labels[0]
            ...
            for i, idx in enumerate(indices):
                ...
                similarity = float(distances[0][i])  # Convert to float for score calculation
            ...
            if best_matches:
                ...
                logger.info("Selected prompt", extra={
                    "action": "select_prompt",
            ...
            logger.warning("No matching prompts found", extra={
                "action": "select_prompt",
                "reason": "no_matches"
            })
            return 'physics', 'generic'  # Return default values
    ...
    def get_prompt_content(self, subject: str, topic: str, user_topic: str | None = None) -> str:
        ...
        try:
            ...
            if not prompt:
                logger.warning("Prompt not found", extra={
                ...
                return self.prompt_generator.generate_from_topic(
                    subject=subject,
                    topic=user_topic or topic  # Use user_topic if provided, otherwise use topic
                )

            # If user_topic is provided, use it to customize the prompt
            if user_topic and user_topic != topic:
                logger.info("Customizing prompt with user topic", extra={
                    "action": "get_prompt_content",
            ...
            logger.info("Using existing prompt", extra={
                "action": "get_prompt_content",
                "prompt_id": prompt.id,
                "subject": subject,
                "topic": topic
            ...
            logger.error("Error getting prompt content", extra={
                "action": "get_prompt_content",
                "subject": subject,
                "topic": topic,
                "error": str(e)
    ...
    def update_index(self):
        ...
        try:
            ...
            self.prompts = self.db.query(Prompt).all()
            if not self.prompts:
                logger.warning("No prompts found in database", extra={
                    "action": "update_index",
                    "reason": "no_prompts"
                })
                return
            ...
            logger.info("FAISS index updated", extra={
                "action": "update_index",
                "prompt_count": len(self.prompts),
                "dimension": dimension
            })
        except Exception as e:
            logger.error("Error updating FAISS index", extra={
                "action": "update_index",
                "error": str(e)
            })
            raise
    ...
    def generate_embeddings_for_prompts(self) -> None:
        ...
        try:
            prompts_without_embeddings = [p for p in self.prompts if p.embedding is None]
            if not prompts_without_embeddings:
                logger.info("No prompts need embeddings", extra={"action": "generate_embeddings"})
                return
