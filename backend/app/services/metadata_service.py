"""Service for managing snippet metadata."""
...
class MetadataService:
    """Service for managing snippet metadata operations."""
    ...
    async def get_by_hash(self, snippet_hash: str) -> Optional[SnippetMetadata]:
        ...
        return self.db.query(SnippetMetadata).filter_by(snippet_hash=snippet_hash).first()
    
    async def update_snippet(self, snippet_hash: str, update_data: Dict[str, Any]) -> Optional[SnippetMetadata]:
        """Update an existing snippet."""
        try:
            snippet = self.db.query(SnippetMetadata).filter_by(snippet_hash=snippet_hash).first()
            if snippet:
