...
class RAGService:
    ...
    async def get_similar_visualizations(
        self, query: str, db, limit: int = 5
    ) -> List[Dict[str, Any]]:
        """Get similar visualizations based on the query text."""
        try:
            query_embedding = EmbeddingService.generate_embedding(query)
            return await self.vector_store.get_similar_visualizations(
                query_embedding, db, limit
            )
            ...
            for viz_type, examples in type_groups.items():
                if examples:
                    ...
                    diverse_examples.append(
                        {"metadata": examples[0].metadata, "type": viz_type}
                    )
    ...
    async def add_snippet_atomic(
        self, text: str, metadata: Dict[str, Any], db: Session
    ) -> None:
        ...
        try:
            ...
            faiss_id = (
                max_faiss_id[0] if max_faiss_id and max_faiss_id[0] is not None else 0
            ) + 1
        ...
        try:
            ...
            logger.error(
                f"Error adding to metadata store, vector store may be inconsistent: {str(e)}"
            )
            raise
