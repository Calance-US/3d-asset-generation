...
async def process_with_retry(
    file: UploadFile, upload_id: str, db, max_retries: int = 3
) -> Dict[str, Any]:
    """Process a file with retry logic."""
    for attempt in range(max_retries):
        try:
            return await process_file(file, upload_id, db)
...
async def process_file(file: UploadFile, upload_id: str, db) -> Dict[str, Any]:
        ...
        try:
            ...
            for snippet in validated_config.snippets:
                ...
                try:
                    ...
                    # Generate a new unique faiss_id (max+1)
